...
def retr_imag(gdat, axis, maps, strg, thisindxener=None, thisindxevtt=-1, cmap='Reds', vmin=None, vmax=None, scal=None, tdim=False):
    ...
    if tdim:
        if thisindxener == None:
            maps = maps.T.flatten()#((gdat.numbpixl))
        else:
...
def retr_lprisinddist(gdat, gdatmodi, sind, sampvarb, l, sinddisttype):
    ...
    if sinddisttype[l] == 'atan':
        lprisinddist = sum(log(pdfn_atan(sind, gdat.minmsind[l], gdat.maxmsind[l]))) 
...
def proc_samp(gdat, gdatmodi, strg, raww=False, fast=False):
    ...
    if 'gaus' in spatdisttype:
        ...
        print 'tdpy.util.rbin(lpdfspatpriointp, [10, 10])'
        set_printoptions(precision=1)
        #print tdpy.util.rbin(lpdfspatpriointp, [10, 10]) - amin(tdpy.util.rbin(lpdfspatpriointp, [10, 10]))
        print lpdfspatpriointp[:20, :20]
