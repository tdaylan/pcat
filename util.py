...
def retr_thisindxprop(gdat, gdatmodi, thisindxpopl=None, brth=False, deth=False):
    ...
    if (rand() < gdat.probtran or brth or deth) and (gdatmodi.thissampvarb[gdat.fittindxfixpnumbpnts[gdatmodi.indxpoplmodi]] != gdat.fittminmnumbpnts[gdatmodi.indxpoplmodi] or \
        ...
        gdatmodi.proppsfnconv = gdat.fittpsfnevaltype == 'conv' and (gdatmodi.indxsampmodi in gdat.fittindxfixppsfp)
        if gdat.elemtype == 'lens':
...
def updt_samp(gdat, gdatmodi):
    ...
    if gdat.fitthostemistype != 'none':
        if gdatmodi.prophost:
            gdatmodi.thissbrtdiff['host'] = copy(gdatmodi.nextsbrtdiff['host'])
...
### find the total surface brightness due to all point sources
def retr_sbrtpntstotl(gdat, lgal, bgal, spec, psfnintp, oaxitype, evalcirc):

    sbrtpnts = zeros((gdat.numbener, gdat.numbpixl, gdat.numbevtt))
    
    numbpnts = lgal.size
    for k in range(numbpnts):
        indxpixleval = retr_indxpixleval(gdat, lgal[k], bgal[k], spec[gdat.indxenerpivt[0], k], evalcirc)
        sbrtpnts[:, indxpixleval, :] += retr_sbrtpnts(gdat, lgal, bgal, spec, psfnintp, oaxitype, indxpixleval)
...
def retr_hubbpsfn(gdat):
    ...
    # temp
    gdat.exprpsfp = array([0.080, 0.087]) / gdat.anglfact
...
def setpinit(gdat, boolinitsetp=False):
    ...
    gdat.listnamevarbstat
    for name in gdat.fittlistnamediff:
        gdat.listnamevarbstat += ['sbrt' + name]
    ...
    for l in gdat.trueindxpopl:
        ...
        setattr(gdat, 'corrcmplpop%d' % l, 1.)
        setattr(gdat, 'factcmplpop%dplot' % l, 1.)
    ...
    gdat.listnamechro = ['totl', 'type', 'prop', 'diag', 'save', 'plot', 'proc', 'lpri', 'llik', 'sbrtmodl']
    if gdat.elemtype == 'lght' or gdat.elemtype == 'clus':
        gdat.listnamechro += ['lghtpntsprep', 'lghtpntseval']
    if gdat.elemtype == 'lens':
        gdat.listnamechro += ['deflzero', 'deflhost', 'deflextr', 'deflelem', 'sbrtlens', 'sbrthost']
    if gdat.fittpsfnevaltype == 'conv' or gdat.truepsfnevaltype == 'conv':
        gdat.listnamechro += ['psfnconv', 'sbrtdiffconv']
    gdat.listnamechro += ['expo', 'llikcalc']
    
    gdat.listlegdchro = ['Total', 'Type', 'Proposal', 'Diagnostics', 'Save', 'Plot', 'Process', 'Prior', 'Posterior', 'Total emission']
    if gdat.elemtype == 'lght' or gdat.elemtype == 'clus':
        gdat.listlegdchro += ['PS Prep', 'Eval. PSF Kernel']
    if gdat.elemtype == 'lens':
        gdat.listlegdchro += ['Array initialization', 'Host Deflection', 'External deflection', 'Subhalo deflection', 'Lensed emission', 'Host emission']
    if gdat.fittpsfnevaltype == 'conv' or gdat.truepsfnevaltype == 'conv':
        gdat.listlegdchro += ['Img for PSF Conv.', 'Conv. w// PSF']
    ...
    gdat.minmexpo = amin(gdat.expo[where(gdat.expo > 1e-50)])
    ...
    for nametruefilt in gdat.listnametruefilt:
        indxtrueelemfilt = [[] for l in range(gdat.truenumbpopl)]
        if nametruefilt == '':
            for l in gdat.trueindxpopl:
                indxtrueelemfilt[l] = arange(gdat.truenumbpnts[l])
                ...
                if strgpdfn == 'tmplgrad':
                    ...
                    for k in range(gdat.numbsidecart + 1):
                        for n in range(gdat.numbsidecart + 1):
                            # temp
                            temp = retr_rele(gdat, gdat.truecntplens[0, :, 0], gdat.binslgal[k], gdat.binsbgal[n], 1., \
...
def retr_datatick(gdat):
    ...
    # data count limits
    gdat.maxmcntpdata = amax(sum(gdat.cntpdata, 2))
...
def retr_indxsamp(gdat, strgmodl='fitt'):
    ...
    setattr(gdat, strgmodl + 'numbback', numbback)
    ...
    if numbtrap > 0:
        ...
        dicttemp['indxfixphypr'] = array(list(dicttemp['indxfixpdist']) + list(dicttemp['indxfixpmeanpnts']))
...
def supr_fram(gdat, gdatmodi, strg, axis, indxpoplplot=-1):
    ...
    if gdat.truelgal != None and gdat.truebgal != None and gdat.truenumbtrap > 0 and gdat.fittnumbtrap > 0:
        if indxpoplplot == -1:
...
def proc_samp(gdat, gdatmodi, strg, raww=False, fast=False, lprionly=False):
    ...
    if numbtrap > 0:
        ...
        if gdat.elemtype == 'lght':
            for l in range(numbpopl):
                dicttemp['spec'][l] = retr_spec(gdat, dicttemp['flux'][l], dicttemp['sind'][l], dicttemp['curv'][l], dicttemp['expc'][l], spectype=spectype[l])
    ...
    if not lprionly:
        ...
        if lensmodltype != 'none':
            ...
            # subhalo deflection field
            if gdat.elemtype == 'lens' and numbpntsconc > 0:
                initchro(gdat, gdatmodi, strg, 'deflelem')
                ...
                stopchro(gdat, gdatmodi, strg, 'deflelem')
        ...
        # point source surface brightness
        if (gdat.elemtype == 'lght' or gdat.elemtype == 'clus') and numbtrap > 0:
            
            initchro(gdat, gdatmodi, strg, 'lghtpntsprep')
            ...
            if strg == 'next' and gdat.pertmodleval and gdat.propwithsing and not gdatmodi.proppsfp:
                ...
                if gdat.verbtype > 1:
                    print 'Preparing only the perturbed point source...'
                if gdatmodi.propfixp:
                    numbelemeval = 0
                ...
                for namecomp in gdat.listnamefeateval:
                    dicttemp[namecomp + 'eval'] = dicttemp[namecomp + 'conc']
            stopchro(gdat, gdatmodi, strg, 'lghtpntsprep')
        ...
        if (gdat.elemtype == 'lght' or gdat.elemtype == 'clus') and numbtrap > 0:
            # evaluate the PSF for each PS in the array
            if gdat.verbtype > 1:
                print 'Evaluating the PSF kernel...'
                print 'sbrtpnts'
                summgene(sbrtpnts)
            initchro(gdat, gdatmodi, strg, 'lghtpntseval')
            listindxpixleval, listindxpixlevalconc = retr_indxpixlevalconc(gdat, numbelemeval, dicttemp, gdat.evalcirc, oaxitype, psfnintp)
        ...
        if (gdat.elemtype == 'lght' or gdat.elemtype == 'clus') and numbtrap > 0:
            if strg == 'next' and gdat.pertmodleval and gdat.propwithsing and not gdatmodi.proppsfp:
                if gdatmodi.propfixp:
                    sbrtpnts = gdatmodi.thissbrtpnts[gdatmodi.indxenermodi, gdatmodi.indxpixleval, gdatmodi.indxevttmodi]
                if gdatmodi.propcomp or gdatmodi.proptran:
                    sbrtpnts = copy(gdatmodi.thissbrtpnts[gdatmodi.indxenermodi, gdatmodi.indxpixleval, gdatmodi.indxevttmodi])
            else:
        ...
        if hostemistype != 'none':
            ...
            # temp -- is this needed?
            #sbrtdiff['host'] = sbrtdiff['host'].reshape((gdat.numbener, gdat.numbpixl, gdat.numbevtt))
        ...
        sbrtmodl = copy(sbrtdiffconv['back0000'])
        ## add other background emission
        for namediff in listnamediff:
            sbrtmodl += sbrtdiffconv[name]
    ...
    if strg != 'next':
        ...
        if (gdat.elemtype == 'lght' or gdat.elemtype == 'clus') and numbtrap > 0:
            ...
            if gdat.calcerrr and gdat.fittnumbtrap > 0:
                ...
                for namecomp in gdat.listnamefeateval:
                    dicttemp[namecomp + 'eval'] = dicttemp[namecomp + 'conc']
                listindxpixleval, listindxpixlevalconc = retr_indxpixlevalconc(gdat, numbelemeval, dicttemp, 'full', sbrtpntsfull, oaxitype, psfnintp)
                for k in range(numbelemeval):
                    sbrtpntsfull[:, indxpixleval, :] += retr_sbrtpnts(gdat, dicttemp['lgaleval'][k], dicttemp['bgaleval'][k], varbevalextd, psfnintp, oaxitype, indxpixleval)
        ...
        if hostemistype != 'none':
            cntphost = retr_cntp(gdat, sbrtdiff['host'])
            setattr(gdatobjt, strg + 'cntphost', cntphost)
            sbrthostmean = retr_spatmean(gdat, sbrtdiff['host'])
        ...
        if lensmodltype != 'none':
            ...
            cntplens = retr_cntp(gdat, sbrtdiff['lens'])
            ...
            sbrtlensmean = retr_spatmean(gdat, sbrtdiff['lens'])
        ...
        if lensmodltype != 'none':
            ...
            for i in gdat.indxener:
                for m in gdat.indxevtt:
                    cntplensgrad[i, :, :, m, :] = retr_gradmaps(gdat, cntplens[i, :, m]) * gdat.sizepixl
        ...
        if numbtrap > 0:
            ...
            if gdat.elemtype == 'lens':
                ...
                for l in indxpopl:
                    ...
                    for k in arange(numbpnts[l]):
                        ...
                        dicttemp['rele'][l][k] = retr_rele(gdat, cntplens[0, :, 0], dicttemp['lgal'][l][k], dicttemp['bgal'][l][k], 1., asca, acut, gdat.indxpixl)
                        dicttemp['reln'][l][k] = dicttemp['rele'][l][k] / dicttemp['defs'][l][k] * gdat.sizepixl
                        dicttemp['reld'][l][k] = retr_rele(gdat, gdat.cntpdata[0, :, 0], dicttemp['lgal'][l][k], dicttemp['bgal'][l][k], 1., asca, acut, gdat.indxpixl)
                        dicttemp['relc'][l][k] = -retr_rele(gdat, cntplens[0, :, 0], dicttemp['lgal'][l][k], dicttemp['bgal'][l][k], 1., asca, acut, gdat.indxpixl, absv=False)
            ...
            listindxmodlelemfilt = []
            for namemodlfilt in gdat.listnamemodlfilt:
                indxmodlelemfilt = [[] for l in gdat.fittindxpopl]
                if namemodlfilt == '':
                    for l in gdat.fittindxpopl:
                        indxmodlelemfilt[l] = where((fabs(dicttemp['lgal'][l]) < gdat.maxmgangdata) & (fabs(dicttemp['bgal'][l]) < gdat.maxmgangdata))[0]
                listindxmodlelemfilt.append(indxmodlelemfilt)
                ...
                for l in indxpopl:
                    if strgfeat == 'specplot' or strgfeat == 'deflprof':
                        ...
                        if strgfeat[:-4] in gdat.listnamefeatsele and strgmodl == 'true':
                            indx = intersect1d(getattr(gdat, strgmodl + 'indxelem' + strgfeat[-4:])[l], listindxmodlelemfilt[0][l])
                            if indx.size > 0:
                                dicttemp['hist' + strgfeat][l, :] = histogram(dicttemp[strgfeat[:-4]][l][indx], bins)[0]
                        else:
                            # temp
                            try:
                                dicttemp['hist' + strgfeat][l, :] = histogram(dicttemp[strgfeat][l][listindxmodlelemfilt[0][l]], bins)[0]
                            except:
                                print 'hey'
                                print 'histograming failed'
                                print 'strgfeat'
                                print strgfeat
                                summgene(dicttemp[strgfeat][l][listindxmodlelemfilt[0][l]])
                                print dicttemp[strgfeat][l][listindxmodlelemfilt[0][l]]
                                print 'bins'
                                print bins
                                print
                                raise Exception('')
                ...
                for a, strgfrst in enumerate(liststrgfeatcorr[l]):
                    for b, strgseco in enumerate(liststrgfeatcorr[l]):
                        if a < b:
                            ...
                            dicttemp['hist' + strgfrst + strgseco][l, :, :] = histogram2d(dicttemp[strgfrst][l][listindxmodlelemfilt[0][l]], \
                                                                                             dicttemp[strgseco][l][listindxmodlelemfilt[0][l]], [binsfrst, binsseco])[0]
        ...
        if numbtrap > 0:
            ...
            # temp
            if strg != 'true' and gdat.trueinfo and gdat.truelgal != None and gdat.truebgal != None:
                ...
                for l in gdat.trueindxpopl:
                    indxmodlelem = zeros(gdat.truenumbpnts[l], dtype=int) - 1
                    ...
                    for k in range(numbpnts[l]):
                        ...
                        if trueindxpntstemp.size > 0:
                            ...
                            indxmodlelem[trueindxpntstemp[0]] = k
                
                    # collect the associated element features
                    indx = where(indxmodlelem >= 0)[0]
                    for strgfeat in liststrgfeatodim[l]:
                        featassc[strgfeat][l] = zeros(gdat.truenumbpnts[l])
                        featassc[strgfeat][l][indx] = dicttemp[strgfeat][l][indxmodlelem[indx]]
...
def retr_indxpixlevalconc(gdat, numbelemeval, dicttemp, evalcirc, sbrtpnts, oaxitype, psfnintp):
