...
def cdfn_dpow(para, minm, maxm, brek, sloplowr, slopuppr):
    ...
    if indxlowr.size > 0:
        cdfn[indxlowr] = facb * (para[indxlowr]**(1. - sloplowr) - minm[indxlowr]**(1. - sloplowr))
    if indxuppr.size > 0:
        cdfnbrek = facb * (brek**(1. - sloplowr) - minm**(1. - sloplowr))
        cdfn[indxuppr] = cdfnbrek + faca / (1. - slopuppr) * (para[indxuppr]**(1. - slopuppr) - brek[indxuppr]**(1. - slopuppr))
...
def icdf_trap(gdat, strgmodl, cdfn, sampvarb, scalcomp, strgcomp, l, d):
    ...
    if scalcomp == 'self' or scalcomp == 'expo' or scalcomp == 'powrslop' or scalcomp == 'dpowslopbrek':
        ...
        if scalcomp == 'powrslop':
            ...
            maxm = getattr(gdat, strgmodl + 'maxm' + strgcomp)
            distslopbrek = sampvarb[getattr(gdat, strgmodl + 'indxfixp' + strgcomp + 'distbrek')[l]]
            ...
            icdf = icdf_dpow(cdfn, minm, maxm, distslopbrek, distsloplowr, distslopuppr)
        else:
    ...
    for k, strgcomp in enumerate(gdat.fittliststrgcomp[gdatmodi.indxpoplmodi[0]]):
        ...
        if listscalcomp[k] == 'self' or listscalcomp[k] == 'dexp' or listscalcomp[k] == 'expo' or listscalcomp[k] == 'powrslop' or listscalcomp[k] == 'dpowslopbrek':
            ...
            if listscalcomp[k] == 'powrslop':
                ...
                maxm = getattr(gdat, 'fittmaxm' + strgcomp)
                distslopbrek = gdatmodi.thissampvarb[getattr(gdat, 'fittindxfixp' + strgcomp + 'distbrek')[gdatmodi.indxpoplmodi[0]]]
            ...
            for g, strgcomp in enumerate(gdat.fittliststrgcomp[l]):
                ...
                try:
                    ...
                    if gdat.fittlistscalcomp[l][g] == 'dpowslopbrek':
                        slopbrek = gdatmodi.thissampvarb[getattr(gdat, 'fittindxfixp' + strgcomp + 'distbrek')[l]]
...
def retr_spec(gdat, flux, sind=None, curv=None, expc=None, sindcolr=None, elin=None, edis=None, spectype='powr', plot=False):
                ...
                if listscalcomptemp[k] == 'powrslop' or listscalcomptemp[k] == 'gaus' or listscalcomptemp[k] == 'dpowslopbrek':
                    if listscalcomptemp[k] == 'powrslop' or listscalcomptemp[k] == 'igam':
                        ...
                        if listscalcomptemp[k] == 'dpowslopbrek':
                            minm = getattr(gdat, 'fittminm' + strgcomp)
                            maxm = getattr(gdat, 'fittmaxm' + strgcomp)
                            slopbrek = nextsampvarb[getattr(gdat, 'fittindxfixp' + strgcomp + 'distbrek')[l]]
                            ...
                            unit = cdfn_powr(comp, minm, maxm, brek, sloplowr, slopuppr)
...
def prop_stat(gdat, gdatmodi, strgmodl, thisindxelem=None, thisindxpopl=None, thisindxregi=None, brth=False, deth=False):
    ...
    if gdatmodi.proptran:
        gdatmodi.nextauxipara = rand(numbcomp[gdatmodi.indxpoplmodi[0]])
    ...
    if gdatmodi.propbrth:
        ...
        nextsamp[gdatmodi.indxsamptran[0]] = gdatmodi.nextauxipara
    ...
    if gdatmodi.propsplt:
        ...
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            if boolcompposi[gdatmodi.indxpoplmodi[0]][g]:
                gdatmodi.nextauxipara[g] = randn() * gdat.radispmr
            elif g == indxcompampl[gdatmodi.indxpoplmodi[0]]:
                gdatmodi.nextauxipara[g] = rand()
            else:
                gdatmodi.nextauxipara[g] = icdf_trap(gdat, strgmodl, rand(), thissampvarb, listscalcomp[gdatmodi.indxpoplmodi[0]][g], \
        ...
        if elemtype[gdatmodi.indxpoplmodi[0]] == 'lghtline':
            gdatmodi.compfrst[0] = gdatmodi.comppare[0] + (1. - gdatmodi.nextauxipara[1]) * gdatmodi.nextauxipara[0]
        else:
            gdatmodi.compfrst[0] = gdatmodi.comppare[0] + (1. - gdatmodi.nextauxipara[2]) * gdatmodi.nextauxipara[0]
            gdatmodi.compfrst[1] = gdatmodi.comppare[1] + (1. - gdatmodi.nextauxipara[2]) * gdatmodi.nextauxipara[1]
        gdatmodi.compfrst[indxcompampl[gdatmodi.indxpoplmodi[0]]] = gdatmodi.nextauxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]] * \
                                                                                                        gdatmodi.comppare[indxcompampl[gdatmodi.indxpoplmodi[0]]]
        if elemtype[gdatmodi.indxpoplmodi[0]] == 'lghtline':
            gdatmodi.compseco[0] = gdatmodi.comppare[0] - gdatmodi.nextauxipara[1] * gdatmodi.nextauxipara[0]
        else:
            gdatmodi.compseco[0] = gdatmodi.comppare[0] - gdatmodi.nextauxipara[2] * gdatmodi.nextauxipara[0]
            gdatmodi.compseco[1] = gdatmodi.comppare[1] - gdatmodi.nextauxipara[2] * gdatmodi.nextauxipara[1]
        gdatmodi.compseco[indxcompampl[gdatmodi.indxpoplmodi[0]]] = (1. - gdatmodi.nextauxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]]) * \
                                                                                                        gdatmodi.comppare[indxcompampl[gdatmodi.indxpoplmodi[0]]]
        for g in range(numbcomp[gdatmodi.indxpoplmodi[0]]):
            if not boolcompposi[gdatmodi.indxpoplmodi[0]][g] and g != indxcompampl[gdatmodi.indxpoplmodi[0]]:
                ...
                gdatmodi.compseco[g] = gdatmodi.nextauxipara[g]
    ...
    if gdatmodi.propmerg:
        ...
        if elemtype[gdatmodi.indxpoplmodi[0]] == 'lghtline':
            gdatmodi.nextauxipara[0] = gdatmodi.compseco[0] - gdatmodi.compfrst[0]
        else:
            gdatmodi.nextauxipara[0] = gdatmodi.compseco[0] - gdatmodi.compfrst[0]
            gdatmodi.nextauxipara[1] = gdatmodi.compseco[1] - gdatmodi.compfrst[1]
        gdatmodi.nextauxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]] = gdatmodi.compfrst[indxcompampl[gdatmodi.indxpoplmodi[0]]] / \
                                        (gdatmodi.compfrst[indxcompampl[gdatmodi.indxpoplmodi[0]]] + gdatmodi.compseco[indxcompampl[gdatmodi.indxpoplmodi[0]]]) 
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            if not boolcompposi[gdatmodi.indxpoplmodi[0]][g] and g != indxcompampl[gdatmodi.indxpoplmodi[0]]:
                gdatmodi.nextauxipara[g] = gdatmodi.compseco[g]
        ...
        if elemtype[gdatmodi.indxpoplmodi[0]] == 'lghtline':
            gdatmodi.comppare[0] = gdatmodi.compfrst[0] + (1. - gdatmodi.nextauxipara[1]) * (gdatmodi.compseco[0] - gdatmodi.compfrst[0])
        else:
            gdatmodi.comppare[0] = gdatmodi.compfrst[0] + (1. - gdatmodi.nextauxipara[2]) * (gdatmodi.compseco[0] - gdatmodi.compfrst[0])
            gdatmodi.comppare[1] = gdatmodi.compfrst[1] + (1. - gdatmodi.nextauxipara[2]) * (gdatmodi.compseco[1] - gdatmodi.compfrst[1])
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            if boolcompposi[gdatmodi.indxpoplmodi[0]][g]:
                gdatmodi.comppare[g] = gdatmodi.compfrst[g] + (1. - gdatmodi.nextauxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]]) * (gdatmodi.compseco[g] - gdatmodi.compfrst[g])
            elif g == indxcompampl[gdatmodi.indxpoplmodi[0]]:
    ...
    if gdat.verbtype > 1 and (gdatmodi.propsplt or gdatmodi.nextaccpprio and gdatmodi.propmerg):
        ...
        if elemtype[gdatmodi.indxpoplmodi[0]] == 'lghtline':
            ...
            print 'auxipara[0]: ', gdatmodi.nextauxipara[0]
            print 'auxipara[1]: ', gdatmodi.nextauxipara[1]
        else:
            ...
            print 'auxipara[0]: ', gdat.anglfact * gdatmodi.nextauxipara[0]
            print 'auxipara[1]: ', gdat.anglfact * gdatmodi.nextauxipara[1]
            print 'auxipara[2]: ', gdatmodi.nextauxipara[2]
    ...
    if gdatmodi.propbrth:
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            nextsampvarb[gdatmodi.indxsamptran[0][g]] = icdf_trap(gdat, strgmodl, gdatmodi.nextauxipara[g], thissampvarb, listscalcomp[gdatmodi.indxpoplmodi[0]][g], \
                                                                            liststrgcomp[gdatmodi.indxpoplmodi[0]][g], gdatmodi.indxpoplmodi[0], gdatmodi.indxregimodi[0])
...
def setpprem(gdat):
    ...
    if gdat.datatype == 'inpt':
        ...
        if gdat.pixltype == 'cart':
            for d in gdat.indxregi:
                if gdat.forccart:
                    sbrtdatatemp = empty((gdat.sbrtdata.shape[0], gdat.numbsidecart, gdat.numbsidecart, gdat.sbrtdata.shape[2]))
                    for i in arange(gdat.sbrtdata.shape[1]):
                        for m in arange(gdat.sbrtdata.shape[3]):
                            sbrtdatatemp[i, :, :, m] = tdpy.util.retr_cart(gdat.sbrtdata[d][i, :, m], numbsidelgal=gdat.numbsidecart, numbsidebgal=gdat.numbsidecart, \
...
def setpinit(gdat, boolinitsetp=False):
        ...
        for l in indxpopl:
            for strgfeat in liststrgfeat[l]:
                ...
                if strgfeat == 'flux' or strgfeat == 'expo' or strgfeat == 'magf' or \
                                     ...
                                     strgfeat == 'relm' or strgfeat == 'dlos' or strgfeat == 'lum0' or \
    ...
    for d in gdat.indxregi:
        if gdat.expo[d].ndim != 3:
            print 'key'
            booltemp = True
        if gdat.pixltype == 'cart' and gdat.expo[d].shape[1] != gdat.numbsidecart**2:
            print 'hey'
            booltemp = True
...
def retr_fromgdat(gdat, gdatmodi, strgstat, strgmodl, strgvarb, mometype='pmea', indxvarb=None, indxlist=None):
    ...
    for strg, k in gdat.__dict__.iteritems():
        ...
        if 'pop' in strg:
            ...
            if namefixp[k].startswith('lum0distsloplowr'):
                lablfixp[k] = r'$\beta_{1,%s%s}$' % (strgpoplcomm, gdat.labllum0)
                scalfixp[k] = 'logt'
            
            if namefixp[k].startswith('lum0distslopuppr'):
                lablfixp[k] = r'$\beta_{2,%s%s}$' % (strgpoplcomm, gdat.labllum0)
                scalfixp[k] = 'logt'
...
def retr_deflextr(gdat, sher, sang):
    ...
    defllgal = factcosi * gdat.lgalgridrofi + factsine * gdat.bgalgridrofi
    deflbgal = factsine * gdat.lgalgridrofi - factcosi * gdat.bgalgridrofi
...
def writfile(gdattemp, path):
    ...
    for attr, valu in gdattemp.__dict__.iteritems():
        ...
        if isinstance(valu, ndarray) and valu.dtype != dtype('O') or isinstance(valu, str) or \
            ...
            if attr != 'adisobjt' and attr != 'redsfromdlosobjt':
...
def proc_samp(gdat, gdatmodi, strgstat, strgmodl, raww=False, fast=False):
                ...
                for d in indxregipopl[l]:
                    for g, strgcomp in enumerate(liststrgcomp[l]):
                        if (listscalcomp[l][g] != 'gaus' and not listscalcomp[l][g].startswith('lnor')) and  \
                                            (dictelem[l][d][strgcomp] < getattr(gdat, strgmodl + 'minm' + strgcomp)).any() or \
    ...
    if evalllik or strgstat != 'next':
        ...
        if lensmodltype != 'none':
            ...
            defl = [zeros((gdat.numbpixl, 2)) for d in indxregieval]
        ...
        if lensmodltype != 'none':
            ...
            for dd, d in enumerate(indxregieval):
                if strgstat == 'next' and not gdatmodi.prophost:
                    ...
                    deflhost[dd] = getattr(gdatobjt, strgpfixthis + 'deflhostreg%d' % d)
                else:
                    if gdat.verbtype > 1:
                    ...
                    deflhost[dd] = retr_defl(gdat, gdat.indxpixl, lgalhost[d], bgalhost[d], beinhost[d], ellp=ellphost[d], angl=anglhost[d])
                     
                    if gdat.diagmode:
                        chec_prop(gdat, gdatobjt, strgstat, strgmodl, strgpfixthis + 'deflhostreg%d' % d, deflhost[dd], [slice(None), slice(None)])
                    ...
                    for l in gdat.fittindxpopl:
                        ...
                        for strgfeat in gdat.liststrgfeatcomm[q][l]:
                            ...
                            if strgfeat == gdat.fittnamefeatampl[l] and len(indxelemfittasschits[q][l][d]) > 0:
                                ...
                                fittfeattemp = dictelem[l][d][name][indxelemfittasschits[q][l][d]]
                        ...
                        for l in gdat.fittindxpopl:
                            for d in gdat.fittindxregipopl[l]:
                                ...
                                for strgfeatfrst, strgfeatfrsttagg in zip(gdat.refrliststrgfeat[q], gdat.refrliststrgfeattagg[q][l]):
                                    ...
                                    if len(indxelemrefrasschits[q][l][d]) > 0:
                                        refrhistfeatfrst = getattr(gdat, 'refrhist' + strgfeatfrst + 'ref%dreg%d' % (q, d))
                                        binsfeatfrst = getattr(gdat, 'bins' + strgfeatfrst)
                                        refrhistfeatfrstassc = histogram(refrfeatfrst[q][d][0, indxelemrefrasschits[q][l][d]], bins=binsfeatfrst)[0]
                            ...
                            for d in gdat.fittindxregipopl[l]:
                                ...
                                for strgfeatfrst, strgfeatfrsttagg in zip(gdat.refrliststrgfeat[q], gdat.refrliststrgfeattagg[q][l]):
                                    ...
                                    for strgfeatseco, strgfeatsecotagg in zip(gdat.refrliststrgfeat[q], gdat.refrliststrgfeattagg[q][l]):
                                        ...
                                        if len(indxelemrefrasschits[q][l][d]) > 0:
                                            fitthistfeattdim =  dictelem[l][d]['hist' + strgfeatfrsttagg + strgfeatsecotagg]
                                            binsfeatseco = getattr(gdat, 'bins' + strgfeatseco)
                                            fitthistfeattdimfals = histogram2d(dictelem[l][d][strgfeatfrst][indxelemfittasscfals[q][l][d]], \
                                            ...
                                            if indxgood[0].size > 0:
                                                ...
                                                if gdat.diagmode:
                                                    if where((fdistdim[indxgood] > 1.) | (fdistdim[indxgood] < 0.))[0].size > 0:
                                                        raise Exception('')
                                        setattr(gdatobjt, strgpfix + 'fdis' + strgfeatfrst + strgfeatseco + 'ref%dpop%dreg%d' % (q, l, d), fdistdim)
                                
                                    fdisfeatfrst = zeros(gdat.numbbinsplot) - 1.
                                    if len(indxelemrefrasschits[q][l][d]) > 0:
                                        refrhistfeatfrst = getattr(gdat, 'refrhist' + strgfeatfrst + 'ref%dreg%d' % (q, d))
                                        binsfeatfrst = getattr(gdat, 'bins' + strgfeatfrst)
                                        fitthistfeatfrstfals = histogram(dictelem[l][d][strgfeat][indxelemfittasscfals[q][l][d]], bins=binsfeatfrst)[0]
                                        ...
                                        if indxgood.size > 0:
                                            ...
                                            if gdat.diagmode:
                                                if where((fdisfeatfrst[indxgood] > 1.) | (fdisfeatfrst[indxgood] < 0.))[0].size > 0:
                                                    raise Exception('')
                                    setattr(gdatobjt, strgpfix + 'fdis' + strgfeatfrst + 'ref%dpop%dreg%d' % (q, l, d), fdisfeatfrst)
...
def proc_finl(gdat=None, rtag=None, strgpdfn='post'):
    ...
    if boolgdatfinl:
        ...
        ## internal correction
        gdat.boolcrin = gdat.datatype == 'inpt' and gdat.pathoutprtagmock != None
        ...
        if gdat.datatype == 'inpt' and gdat.boolcorr:
            for liststrgcompvarbhist in gdat.liststrgvarbhist:
                ...
                listhist = getattr(gdat, 'list' + strgvarb)
        ...
        for strgchan in gdat.liststrgchan:
            ...
            if isinstance(listtemp, list):
                ...
                if 'asscref' in strgchan:
                    ...
                    for k in range(listtemp.shape[1]):
                        ...
                        if indxassc.size > 0:
                            posttemp[:, k] = tdpy.util.retr_postvarb(listtemp[indxassc, k])
                            meantemp[k] = mean(listtemp[indxassc, k])
                else:
            ...
            setattr(gdat, 'pmea' + strgchan, meantemp)
            ...
            for l in gdat.fittindxpopl:
                for d in gdat.fittindxregipopl[l]:
                    for strgfeat in gdat.fittliststrgfeat[l]:
                        if strgfeat == 'spec':
                            continue
                        setp_pdfnvarb(gdat, strgfeat, strgfeat + 'pop%dreg%d' % (l, d), strgpdfn)
                ...
                for l in gdat.fittindxpopl:
                    for d in gdat.fittindxregipopl[l]:
                        for strgfeat in gdat.fittliststrgfeat[l]:
                            if strgfeat == 'spec':
                                continue
                            setp_info(gdat, gdatprio, strgfeat, strgfeat + 'pop%dreg%d' % (l, d))
        ...
        # temp
        if not 'listlpautotl' in gdat.__dict__:
            raise Exception('')
...
def setp_info(gdat, gdatprio, name, namefull):
...
def setp_pdfnvarb(gdat, name, namefull, strgpdfn):

    binsvarbscal = getattr(gdat, 'bins' + name)
    meanvarbscal = getattr(gdat, 'mean' + name)
    deltvarbscal = getattr(gdat, 'delt' + name)
    listvarbscal = getattr(gdat, 'list' + name)
    if name in gdat.listnamevarbscal:
        pdfn = histogram(listvarbscal, bins=binsvarbscal)[0].astype(float)
    else:
        pdfn = getattr(gdat, 'pmeahist' + namefull)
...
def plot_samp(gdat, gdatmodi, strgstat, strgmodl):
    ...
    if numbtrap > 0:
        liststrgfeatcorr = getattr(gdat, strgmodl + 'liststrgfeatcorr')
        liststrgfeatcorrplot = getattr(gdat, strgmodl + 'liststrgfeatcorrplot')
        spectype = getattr(gdat, strgmodl + 'spectype')
        elemtype = getattr(gdat, strgmodl + 'elemtype')
        calcerrr = getattr(gdat, strgmodl + 'calcerrr')
        boolelemsbrtextsbgrdanyy = getattr(gdat, strgmodl + 'boolelemsbrtextsbgrdanyy')
    ...
    if not (gdat.shrtfram and strgstat == 'this' and strgmodl == 'fitt'):
        ...
        if numbtrap > 0:
            ...
            # element feature correlations
            for l in gdat.fittindxpopl:
                for a, strgfrst in enumerate(liststrgfeatcorrplot[l]):
                    for b, strgseco in enumerate(liststrgfeatcorrplot[l]):
                        if a < b:
                            for strgplottype in ['hist', 'scat']:
                                ...
                                plot_elemtdim(gdat, gdatmodi, strgstat, strgmodl, d, l, strgplottype, strgfrst, strgseco, strgmome='pmea')
        ...
        if gdat.penalpridiff:
            for d in gdat.indxregi:
                for i in gdat.indxener:
                    for m in gdat.indxevtt:
                        plot_gene(gdat, gdatmodi, strgstat, strgmodl, 'psecodimdatapntsreg%den%02devt%d' % (d, i, m), 'meanmpolodim', lablxdat='$l$', lablydat='$P_{resi}(l)$', \
                                                                                                                 limtydat=[1e-2, 2.], scalxdat='logt', scalydat='logt')
                        plot_gene(gdat, gdatmodi, strgstat, strgmodl, 'psecodimdatapntsprioreg%den%02devt%d' % (d, i, m), 'meanmpolodim', lablxdat='$l$', \
...
def plot_info(gdat, gdatprio, name, namefull):
...
def plot_finl(gdat=None, gdatprio=None, rtag=None, strgpdfn='post'):
    ...
    if gdat.checprio and strgpdfn == 'post':
        ...
        for namevarbscal in gdat.listnamevarbscal:
            plot_info(gdat, gdatprio, namevarbscal, namevarbscal)
        for l in gdat.fittindxpopl:
            for strgfeat in gdat.fittliststrgfeat[l]:
                for d in gdat.fittindxregipopl[l]:
                    if strgfeat == 'spec':
                        continue
                    plot_info(gdat, gdatprio, strgfeat, strgfeat + 'pop%dreg%d' % (l, d))
    ...
    if gdat.fittnumbtrap > 0 and gdat.probspmr > 0.:
        ...
        pathroot = getattr(gdat, 'path' + gdat.namesampdist + 'finlspmr')
    ...
    if gdat.propbacp:
        ...
        if gdat.fittnumbback == 2 and gdat.fittspecback == [False, False]:
            for i in gdat.indxener:
                ...
                path = getattr(gdat, 'path' + gdat.namesampdist + 'finlvarbscalcova') + 'bacpen%02d' % i
        ...
        for a in range(numbstrgstattemp):
            ...
            if gdat.numbener > 1:
                ...
                for k in range(listydat.shape[0]):
                    ...
                    axis.errorbar(xdat, ydat, yerr=yerr, color=colr, marker=mrkr, ls=linestyl, markersize=15, alpha=alph, label=listlegdsbrtspec[cntr])
...
def plot_elemtdim(gdat, gdatmodi, strgstat, strgmodl, indxregiplot, indxpoplplot, strgplottype, strgfrst, strgseco, strgmome='pmea'):
...
def plot_gene(gdat, gdatmodi, strgstat, strgmodl, strgydat, strgxdat, indxrefrplot=None, indxydat=None, strgindxydat=None, indxxdat=None, strgindxxdat=None, plottype='none', \
    ...
    if strgydat.startswith('histcntp'):
        ...
        axis.bar(xdattemp, ydattemp, deltxdat, color='black', label='Data', alpha=gdat.alphmrkr, linewidth=5, edgecolor='black')
            ...
            if strgydat[4:-8] in liststrgfeatprio[int(strgydat[-5])]:
                ...
                if gdat.datatype == 'mock' and not omittrue:
                    for q in gdat.indxrefr:
                        if strgydat[4:-8] in gdat.trueliststrgfeatprio[q]:
                            ...
                            axis.plot(truexdatprio, trueydatsupr, ls='-', alpha=gdat.alphmrkr, color=gdat.refrcolrelem[q])
                ...
                if strgmodl != 'true':
                    if strgstat == 'post':
                        ...
                        axis.plot(xdatprio, ydatsupr, ls='--', alpha=gdat.alphmrkr, color=gdat.fittcolrelem[int(strgydat[-5])])
...
def plot_genemaps(gdat, gdatmodi, strgstat, strgmodl, strgvarb, indxenerplot=None, indxevttplot=-1, strgcbar=None, \
                                                                tdim=False, indxpoplplot=-1, mometype='pmea', intreval=False, indxmaps=None):
