            ...
            if elemtype[l] == 'lghtline':
                ...
                if boolelemspec[l]:
                    if boolelemsbrtextsbgrd[l]:
                        indxsampcomp['gwdt'][l][d] = indxsamptemp + 2
                        indxsampcomp['flux'][l][d] = indxsamptemp + 3
                        if gdat.numbener > 1:
                            indxsampcomp['sind'][l][d] = indxsamptemp + 4
                    else:
                        if elemtype[l] == 'lghtpntspuls':
                            indxsampcomp['per0'][l][d] = indxsamptemp + 2
                            indxsampcomp['magf'][l][d] = indxsamptemp + 3
                            indxsampcomp['dglc'][l][d] = indxsamptemp + 4
                            indxsampcomp['geff'][l][d] = indxsamptemp + 5
                            indxtemp = 6
                        else:
                        ...
                        if gdat.numbener > 1:
                            if spectype[l] == 'colr':
                                for i in gdat.indxener:
                                    if i == 0:
                                        continue
                                    indxsampcomp['sind%04d' % i][l][d] = indxsamptemp + indxtemp + i - 1 
                            else:
                                indxsampcomp['sind'][l][d] = indxsamptemp + indxtemp
                                if spectype[l] == 'curv':
                                    indxsampcomp['curv'][l][d] = indxsamptemp + indxtemp + 1
                                if spectype[l] == 'expc':
                                    indxsampcomp['expc'][l][d] = indxsamptemp + indxtemp + 1
                if elemtype[l] == 'lens':
...
def updt_samp(gdat, gdatmodi):
    ...
    if gdat.fitthostemistype != 'none':
        if gdatmodi.prophost:
            for dd, d in enumerate(gdatmodi.indxregieval):
                gdatmodi.thissbrthost[d][gdatmodi.indxcubeeval[0][dd]] = copy(gdatmodi.nextsbrthost[dd])
    
    if gdat.fittlensmodltype != 'none':
        if gdatmodi.prophost:
            for dd, d in enumerate(gdatmodi.indxregieval):
                gdatmodi.thisdeflhost[d][gdatmodi.indxpixleval[1][dd], :] = copy(gdatmodi.nextdeflhost[dd])
    ...
    if gdatmodi.propelem:
        ...
        if gdat.fittboolelemsbrtdfnc[gdatmodi.indxpoplmodi[0]]:
            for dd, d in enumerate(gdatmodi.indxregieval):
                gdatmodi.thissbrtdfnc[d][gdatmodi.indxcubeeval[0][dd]] = copy(gdatmodi.nextsbrtdfnc[dd])
                if gdat.verbtype > 1:
                    print 'gdatmodi.nextsbrtdfnc[dd]'
                    summgene(gdatmodi.nextsbrtdfnc[dd])
                    print 'gdatmodi.thissbrtdfnc[d]'
                    summgene(gdatmodi.thissbrtdfnc[d])
        if gdat.fittboolelemdeflsubh[gdatmodi.indxpoplmodi[0]]:
            for dd, d in enumerate(gdatmodi.indxregieval):
                gdatmodi.thisdeflsubh[d] = copy(gdatmodi.nextdeflsubh[dd])
        if gdat.fittboolelemsbrtextsbgrd[gdatmodi.indxpoplmodi[0]]:
            for dd, d in enumerate(gdatmodi.indxregieval):
                gdatmodi.thissbrtextsbgrd[d][gdatmodi.indxcubeeval[0][dd]] = copy(gdatmodi.nextsbrtextsbgrd[dd])
                if gdat.verbtype > 1:
                    print 'gdatmodi.nextsbrtextsbgrd[dd]'
                    summgene(gdatmodi.nextsbrtextsbgrd[dd])
                    print 'gdatmodi.thissbrtextsbgrd[d]'
                    summgene(gdatmodi.thissbrtextsbgrd[d])
...
def prop_stat(gdat, gdatmodi, strgmodl, thisindxelem=None, thisindxpopl=None, thisindxregi=None, brth=False, deth=False):
    ...
    if numbtrap > 0 and (deth or brth or rand() < gdat.probtran) and not (numbelemtemp == minmnumbelemtemp and numbelemtemp == maxmnumbelemtemp):
        ...
        if brth or deth or rand() < gdat.probbrde / gdat.probtran or numbelemtemp == 0:
            ...
            ## splits and merges
            if numbelemtemp == maxmnumbelemtemp:
                gdatmodi.propmerg = True
            elif numbelemtemp == minmnumbelemtemp or numbelemtemp < 2:
    ...
    if gdatmodi.propbrth:
        ...
        nextsamp[gdatmodi.indxsamptran[0]] = gdatmodi.auxipara
    ...
    if gdatmodi.propsplt:
        ...
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            if boolcompposi[gdatmodi.indxpoplmodi[0]][g]:
                gdatmodi.auxipara[g] = randn() * gdat.radispmr
            elif g == indxcompampl[gdatmodi.indxpoplmodi[0]]:
                gdatmodi.auxipara[g] = rand()
            else:
                gdatmodi.auxipara[g] = icdf_trap(gdat, strgmodl, rand(), thissampvarb, listscalcomp[gdatmodi.indxpoplmodi[0]][g], \
        ...
        gdatmodi.compseco[indxcompampl[gdatmodi.indxpoplmodi[0]]] = (1. - gdatmodi.auxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]]) * \
                                                                                                        gdatmodi.comppare[indxcompampl[gdatmodi.indxpoplmodi[0]]]
        for g in range(numbcomp[gdatmodi.indxpoplmodi[0]]):
            if not boolcompposi[gdatmodi.indxpoplmodi[0]][g] and g != indxcompampl[gdatmodi.indxpoplmodi[0]]:
                ...
                gdatmodi.compseco[g] = gdatmodi.auxipara[g]
        ...
        if elemtype[gdatmodi.indxpoplmodi[0]] == 'lghtline':
            ...
            if fabs(gdatmodi.compfrst[0]) > gdat.maxmgang or fabs(gdatmodi.compseco[0]) > gdat.maxmgang or \
                                                                    fabs(gdatmodi.compfrst[1]) > gdat.maxmgang or fabs(gdatmodi.compseco[1]) > gdat.maxmgang:
                gdatmodi.thisaccpprio = False
    ...
    if gdatmodi.propmerg:
        ...
        gdatmodi.auxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]] = gdatmodi.compfrst[indxcompampl[gdatmodi.indxpoplmodi[0]]] / \
                                        (gdatmodi.compfrst[indxcompampl[gdatmodi.indxpoplmodi[0]]] + gdatmodi.compseco[indxcompampl[gdatmodi.indxpoplmodi[0]]]) 
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            if not boolcompposi[gdatmodi.indxpoplmodi[0]][g] and g != indxcompampl[gdatmodi.indxpoplmodi[0]]:
                gdatmodi.auxipara[g] = gdatmodi.compseco[g]
        ...
        for g, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
            if boolcompposi[gdatmodi.indxpoplmodi[0]][g]:
                gdatmodi.comppare[g] = gdatmodi.compfrst[g] + (1. - gdatmodi.auxipara[indxcompampl[gdatmodi.indxpoplmodi[0]]]) * (gdatmodi.compseco[g] - gdatmodi.compfrst[g])
            elif g == indxcompampl[gdatmodi.indxpoplmodi[0]]:
                ...
                if gdat.fittboolelemsbrtdfnc[l]:
                    ...
                    for d in gdat.fittindxregipopl[l]:
                        ...
                        temp[d]['spec'] = retr_spec(gdat, temp[d]['flux'], sind=temp[d]['sind'], curv=temp[d]['curv'], \
                                                expc=temp[d]['expc'], sind0001=temp[d]['sind0001'], sind0002=temp[d]['sind0002'], spectype=spectype[l])
...
def setpinit(gdat, boolinitsetp=False):
        ...
        for l in indxpopl:
            for strgfeat in liststrgfeat[l]:
                ...
                if strgfeat.startswith(gdat.namefeatsign) or strgfeat.startswith(namefeatampl[l]) or strgfeat == 'expo' or \
                                     strgfeat == 'cnts' or strgfeat.startswith('per') or strgfeat == 'gwdt' or \
    ...
    if gdat.datatype != 'mock':
        if gdat.fittboolelemspecanyy and gdat.exprtype == 'ferm' and gdat.maxmgangdata == 20. / gdat.anglfact:
    ...
    ## text
    if gdat.allwrefr:
        if gdat.refrlegdpopl == None:
            gdat.refrlegdpopl = [[] for q in gdat.indxrefr]
            for q in gdat.indxrefr:
                gdat.refrlegdpopl[q] = 'Pop %d' % q
        if gdat.refrlegd == None:
            if gdat.datatype == 'mock':
                gdat.refrlegd = 'True'
            else:
                gdat.refrlegd = 'Ref'
        gdat.refrlegdelem = [[] for q in gdat.indxrefr]
        gdat.refrlegdmiss = [[] for q in gdat.indxrefr]
        gdat.refrlegdhits = [[] for q in gdat.indxrefr]
        for q in gdat.indxrefr:
            gdat.refrlegdelem[q] = gdat.refrlegd + ' ' + gdat.refrlegdpopl[q]
            gdat.refrlegdmiss[q] = gdat.refrlegdelem[q] + ' miss'
            gdat.refrlegdhits[q] = gdat.refrlegdelem[q] + ' hit'
        gdat.refrlegdhost = gdat.refrlegd + ' host'
        gdat.refrlegdsour = gdat.refrlegd + ' sour'
    ...
    retr_axis(gdat, 'deflsubh', -gdat.maxmgang * 1e-2, gdat.maxmgang * 1e-2, 40)
    ...
    if 'lghtline' in gdat.fittelemtype:
        ...
        if gdat.fittboolelemspecanyy:
            if gdat.fittboolelemsbrtextsbgrdanyy:
                gdat.indxstdpgwdt = gdat.numbfixpprop + 2
                gdat.indxstdpflux = gdat.numbfixpprop + 3
                gdat.indxstdpsind0001 = gdat.numbfixpprop + 4
                gdat.indxstdpsind0002 = gdat.numbfixpprop + 5
                gdat.indxstdpsind = gdat.numbfixpprop + 4
                gdat.indxstdpcurv = gdat.numbfixpprop + 5
                gdat.indxstdpexpc = gdat.numbfixpprop + 5
            else:
    ...
    for k in gdat.indxstdp:    
        ...
        if indx.size > 0:
            ...
            #print 'indx'
            #print indx
            #print 'gdat.fittnamepara[indx[0]]'
            #print gdat.fittnamepara[indx[0]]
    ...
    if gdat.boolhash:
        ...
        if gdat.commboolelempsfnanyy and gdat.maxmangl - amax(gdat.maxmangleval) < 1.1 * sqrt(2) * (gdat.maxmgang - gdat.maxmgangdata):
            print 'gdat.maxmangl'
...
def retr_indxsamp(gdat, strgmodl='fitt', init=False):
    ...
    if init:
        ...
        boolelemspecanyy = True in boolelemspec
        ...
        for l in indxpopl:
            ...
            if boolelemspat[l] and boolelemspec[l]:
        ...
        for l in indxpopl:
            if elemtype[l].startswith('lght') and not elemtype[l].endswith('bgrd') or elemtype[l] == 'clus':
        ...
        for l in indxpopl:
            if elemtype[l] == 'lghtpntspuls':
                ...
                setp_varblimt(gdat, 'dglc', [1e-3, 3.], strgmodl=strgmodl, popl=l)
                #setp_varblimt(gdat, 'per1', [1e-22, 1e-10], strgmodl=strgmodl, popl=l)
...
def setp_fixp(gdat, strgmodl='fitt'):
    ...
    for strg, k in gdat.__dict__.iteritems():
        ...
        if 'pop' in strg:
            ...
            if namefixp[k].startswith('gwdtdistslopp'):
                lablfixp[k] = r'$\alpha_{%s%s}$' % (strgpoplcomm, gdat.lablgwdt)
                scalfixp[k] = 'logt'
            ...
            if namefixp[k].startswith('per0distmean'):
                lablfixp[k] = r'$\lambda_{%s%s}$' % (strgpoplcomm, gdat.lablper0)
                scalfixp[k] = 'logt'
            
            if namefixp[k].startswith('per0diststdv'):
                lablfixp[k] = r'$\sigma_{%s%s}$' % (strgpoplcomm, gdat.lablper0)
                scalfixp[k] = 'logt'
            
            if namefixp[k].startswith('magfdistmean'):
                lablfixp[k] = r'$\lambda_{%s%s}$' % (strgpoplcomm, gdat.lablmagf)
                scalfixp[k] = 'logt'
            
            if namefixp[k].startswith('magfdiststdv'):
                lablfixp[k] = r'$\sigma_{%s%s}$' % (strgpoplcomm, gdat.lablmagf)
                scalfixp[k] = 'logt'
            
            if namefixp[k].startswith('dglcdistslop'):
                lablfixp[k] = r'$\alpha_{%s%s}$' % (strgpoplcomm, gdat.labldglc)
                scalfixp[k] = 'logt'
    ...
    for l in indxpopl:
        if boolelemsbrtpnts[l]:
            listlegdsbrt.append('PS')
            listlegdsbrt.append('Unrs. PS')
        elif boolelemsbrtdfnc[l]:
            listlegdsbrt.append('PS')
            listlegdsbrt.append('Unrs. PS')
        elif boolelemsbrt[l]:
            listlegdsbrt.append('Src')
            listlegdsbrt.append('Unrs. Src')
    if 'lens' in elemtype:
...
@jit(nopython=True, nogil=True)
def retr_deflsubh_jitt(deflsubh, indxpixl, lgalgrid, bgalgrid, numbelemconc, lgalconc, bgalconc, defsconc, ascaconc, acutconc):
    
    for k in range(numbelemconc):
        deflsubh[:] += retr_defl_jitt(indxpixl, lgalgrid, bgalgrid, lgalconc[k], bgalconc[k], defsconc[k], 0., 0., asca=ascaconc[k], acut=acutconc[k])
...
def proc_samp(gdat, gdatmodi, strgstat, strgmodl, raww=False, fast=False):
        ...
        for l in indxpopl:
            for d in indxregipopl[l]:
                ...
                for strgcomp in liststrgcomp[l]:
                    ...
                    if gdat.diagmode:
                        if (abs(sampvarb[indxsampcomp[strgcomp][l][d]]) < 1e-100).any():
                            raise Exception('')
        ...
        if strgstat == 'this':# or strgstat == 'next' and (('lght' in elemtype or 'clus' in elemtype) and gdatmodi.proppsfp):
            ...
            for l in indxpopl:
                if elemtype[l].startswith('lght'):
                    for d in indxregipopl[l]:
                        if elemtype[l] == 'lghtpntspuls':
                            dictelem[l][d]['dlos'] = retr_dlosgalx(dictelem[l][d]['lgal'], dictelem[l][d]['bgal'], dictelem[l][d]['dglc'])
    ...
    if evalllik or strgstat != 'next':
        ...
        if psfnevaltype == 'conv' or psfnevaltype == 'full':
            ...
            if strgstat == 'next' and not gdatmodi.proppsfnconv:
                ...
                if gdat.verbtype > 1:
                    print 'Constructing the PSF convolution kernel...'
        ...
        if numbtrap > 0:
            ...
            for ll, l in enumerate(indxpopleval):
                ...
                for yy, y in enumerate([indxgridpopl[l]]):
                    if strgstat == 'next' and gdatmodi.evalllikpert and (elemspatevaltype[l] == 'locl' or elemspatevaltype[l] == 'loclhash') and not boolelemsbrtextsbgrd[l]:
        ...
        if numbtrap > 0:
            ...
            for dd, d in enumerate(indxregieval):
                if boolelemsbrtdfncanyy:
                    if strgstat == 'next' and not gdatmodi.proppsfp:
                        ...
                        if gdatmodi.propelemsbrtdfnc or gdatmodi.proppsfp:
                            sbrtdfnc[dd][indxcubeeval[0][dd]] = copy(getattr(gdatobjt, strgpfixthis + 'sbrtdfnc')[d][indxcubeeval[0][dd]])
                        else:
                            sbrtdfnc[dd][indxcubeeval[0][dd]] = getattr(gdatobjt, strgpfixthis + 'sbrtdfnc')[d][indxcubeeval[0][dd]]
                    else:
                ...
                if boolelemsbrtextsbgrdanyy: 
                    if strgstat == 'next':
                        ...
                        if gdatmodi.propelemsbrtextsbgrd:
                            sbrtextsbgrd[dd][indxcubeeval[0][dd]] = copy(getattr(gdatobjt, strgpfixthis + 'sbrtextsbgrd')[d][indxcubeeval[0][dd]])
                        else:
                            sbrtextsbgrd[dd][indxcubeeval[0][dd]] = getattr(gdatobjt, strgpfixthis + 'sbrtextsbgrd')[d][indxcubeeval[0][dd]]
                    else:
                        sbrtextsbgrd[dd] = zeros_like(gdat.expo[d])
                if boolelemdeflsubhanyy:
                    if strgstat == 'next':
                        if gdatmodi.propelemdeflsubh:
                            deflsubh[dd] = copy(getattr(gdatobjt, strgpfixthis + 'deflsubh')[d])
                        else:
                            deflsubh[dd] = getattr(gdatobjt, strgpfixthis + 'deflsubh')[d]
                    else:
            ...
            if boolelemsbrtdfncanyy:
                ...
                if strgstat == 'this' or gdatmodi.propelemsbrtdfnc:
                    for ll, l in enumerate(indxpopleval):
                        if elemtype[l] != 'lens':
                            for dd, d in enumerate(indxregieval):
                        ...
                        if elemtype[l] == 'lens':
                            for dd, d in enumerate(indxregieval):
                                for kk, k in enumerate(indxelemeval[ll][dd]):
                                    ...
                                    print 'acut'
                                    print acut
                                    print 'deflsubh[dd][listindxpixleval[ll][dd][kk], :]'
                                    summgene(deflsubh[dd][listindxpixleval[ll][dd][kk], :])
                                    print
            ...
            if boolelemsbrtextsbgrdanyy:
                ...
                setattr(gdatobjt, strgpfix + 'sbrtextsbgrd', sbrt['extsbgrd'])
        ...
        if lensmodltype != 'none':
            ...
            if strgstat == 'this' or gdatmodi.propsbrtlens:
        ...
        if hostemistype != 'none':
            ...
            for dd, d in enumerate(indxregieval):
                if strgstat == 'next' and not gdatmodi.prophost:
                    ...
                    sbrt['host'][dd] = getattr(gdatobjt, strgpfixthis + 'sbrthost')[d][indxcubeeval[0][dd]]
                else:
                    if gdat.verbtype > 1:
                    ...
                    if gdat.verbtype > 1:
                        ...
                        print 'serihost[d]'
                        print serihost[d]
                    sbrt['host'][dd] = retr_sbrtsers(gdat, gdat.lgalgrid, gdat.bgalgrid, lgalhost[d], \
        ...
        if convdiffanyy:
            ...
            if strgstat == 'this' or not (numbtrap > 0 and gdatmodi.propelemsbrtdfnc):
                ...
                if gdat.verbtype > 1:
                    print 'Retrieving the convolved model...'
                for dd, d in enumerate(indxregieval):
                    sbrt['modlconv'][dd] = getattr(gdatobjt, strgpfixthis + 'sbrtmodlconv')[d][indxcubeeval[0][dd]]
            
            # state variables should be defined with individual names in the locals dictionary
            sbrtmodlconv = sbrt['modlconv']
            
            sbrt['modl'] = sbrt['modlconv']
        else:
            if gdat.verbtype > 1:
                print 'Skipping PSF convolution of the model...'
            for dd, d in enumerate(indxregieval):
                sbrt['modl'][dd] = sbrt['modlraww'][dd]
        ...
        if gdat.verbtype > 1:
            ...
            print lliktotl
            print
        if False and gdat.diagmode:
            if strgmodl == 'fitt' and strgstat == 'this':
                try:
                    booltemp = True
                    booltemp = booltemp and gdatmodi.cntrswep >= 0
                except:
                    booltemp = False
                if booltemp:
                    print 'Checking state variables...'
                    for attr, valu in gdatmodi.__dict__.iteritems():
                        if not attr.startswith('this') or not attr[4:] in gdat.listnamevarbstat:
                            continue
                        if attr == 'thissamp':
                            continue
                        valuneww = locals()[attr[4:]]
                        booltemptemp = False
                        if type(valu) != type(valuneww):
                            print 'type(valu)'
                            print type(valu)
                            print 'type(valuneww)'
                            print type(valuneww)
                        ...
                        if isinstance(valu, ndarray):
                            if (valuneww != valu).any():
                                booltemptemp = True
                        else:#if isinstance(valu, ):
                            if valuneww != valu:
                                booltemptemp = True
                        if booltemptemp:
                            print 'attr'
            ...
            for d in indxregipopl[l]:
                ...
                for k, (strgfeat, strgpdfn) in enumerate(zip(liststrgfeatprio[l], liststrgpdfnprio[l])):
                    ...
                    if strgpdfn == 'self':
                        ...
                        lpri[indxlpri] = sum(log(pdfn_dexp(dictelem[l][d]['bgal'], maxmgang, sampvarb[indxfixpbgaldistscal]))) 
                        ...
                        lpri[indxlpri] = sum(log(pdfn_expo(gang, maxmgang, sampvarb[indxfixpgangdistscal]))) 
        ...
        if strgstat == 'next':
            ...
            if gdatmodi.proptran:
                ...
                if gdatmodi.propsplt or gdatmodi.propmerg:
                    ...
                    for k, strgcomp in enumerate(liststrgcomp[gdatmodi.indxpoplmodi[0]]):
                        if k == 0 or k == 1:
                            lpau[k] = -log(2. *pi) - log(gdat.radispmr) -0.5 * (gdatmodi.auxipara[k] / gdat.radispmr)
                        elif k == 2:
    ...
    if strgstat != 'next':
            ...
            if gdat.fittnumbtrap > 0:
                ...
                for l in indxpopl:
                    for dd, d in enumerate(indxregipopl[l]):
                        dicteval[l][dd]['deltllik'] = dictelem[l][d]['deltllik']
                    if calcerrr[l]:
                        numbiter = 2
                    else:
                        numbiter = 1
                        ...
                        if boolelemsbrt[l]:
                            for a in range(numbiter):
                                for k in range(numbelemeval[l][dd]):
                                    if boolelemspec[l]:
                                        varbevalextd = dicteval[l][dd]['spec'][:, k]
                                    if elemtype[l] == 'clus':
                                        varbevalextd = dicteval[l][dd]['nobj'][None, k]
                                    if a == 0:
                                        if dicteval[l][dd]['deltllik'][k] < 35:
                                            sbrt['dfncsubt'][d][:, listindxpixleval[l][d][k], :] += retr_sbrtpnts(gdat, dicteval[l][d]['lgal'][k], dicteval[l][d]['bgal'][k], \
                                                                                                                    varbevalextd, psfnintp, oaxitype, listindxpixleval[l][d][k])
                                    if a == 1:
                                        sbrt['dfncfull'][d][:, :, :] += retr_sbrtpnts(gdat, dicteval[l][d]['lgal'][k], dicteval[l][d]['bgal'][k], \
        ...
        if lensmodltype != 'none':
            ...
            for d in gdat.indxregi:
                setattr(gdatobjt, strgpfix + 'deflmgtdreg%d' % d, deflmgtd[d, ...])
...
def retr_flux(gdat, lumi, dlos):

    flux = lumi / 4. / pi / dlos**2 / gdat.kprccmtr**2
